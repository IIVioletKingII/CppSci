...
int main()
{
     ...
     * of the line. The only thing remaining may be an carriage return/enter,  
    ...
    // Run this first with this line commented out. Then uncomment and run again.
    // cin.ignore();
    
    cout << "Now type a phrase: ";
    /* getline reads up to the next <enter>, so we need to make sure
     * we clear the enter from the previous prompt using the cin.ignore()
     * command
     */
    cin.ignore();
    getline(cin, phrase);
